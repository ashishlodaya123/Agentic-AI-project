...
const DifferentialDiagnosis = ({ taskId, patientData, symptomsAnalysis, riskAssessment }) => {
  ...
  if (loading) {
    return (
      <div className="text-center py-8">
        ...
        <p className="body-large text-neutral-text-secondary">Analyzing differential diagnosis possibilities...</p>
      </div>
    );
  }
  ...
  if (!diagnosisData || !diagnosisData.differential_diagnosis || diagnosisData.differential_diagnosis.length === 0) {
    return (
      <div className="text-center py-8">
        <FaBrain className="mx-auto h-12 w-12 text-gray-300 mb-3" />
        <p className="body-large text-neutral-text-secondary">
          No differential diagnosis data available. This feature requires patient symptoms and clinical analysis.
        </p>
      </div>
    );
  }
  ...
  // Sort by match score
  const sortedDiagnoses = [...diagnosisData.differential_diagnosis].sort((a, b) => b.match_score - a.match_score);
          ...
          <div key={index} className="card">
            <div className="card-body">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="h3 text-neutral-text">{diagnosis.condition}</h3>
                  <div className="flex items-center mt-1">
                    <span className={`badge ${
                      diagnosis.severity === 'high' ? 'badge-error' :
                      diagnosis.severity === 'moderate' ? 'badge-warning' :
                      'badge-info'
                    }`}>
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-neutral-text-secondary">Match Score</div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {diagnosis.matched_vitals.map((vital, vitalIndex) => (
                      <span key={vitalIndex} className="badge badge-warning text-xs">
                        {vital.replace('_', ' ')}
                      </span>
                    ))}
                  </div>
              ...
              <div className="mt-4 pt-4 border-t border-neutral-border">
                <h4 className="font-medium text-neutral-text mb-2">Recommended Actions</h4>
                <ul className="list-disc pl-5 space-y-1">
                  {diagnosis.recommendations?.map((rec, recIndex) => (
                    <li key={recIndex} className="body-small text-neutral-text-secondary">
                      {rec}
                    </li>
                  ))}
                </ul>
